...
function ocultarTodo() {
    $('#role-selection-view, #teacher-login-view, #teacher-dashboard-view, #teacher-lobby-view, #setup-view, #student-menu-view, #student-lobby-view, #game-controls, #board-wrapper, #academy-main-menu, #ranking-view, #mesas-area, #chat-area, #online-header, #bot-header, #puzzle-header, #teacher-header, #academy-header, #final-results-view').hide();
    game = new Chess();
    selectedSquare = null;
    removeHighlights();
    playerColor = 'w';
...
if (socket) {
    ...
    function unirsePartida(roomCode) {
    ...
    socket.on('tournamentCreated', function (data) {
        ...
        else $('#manual-add-add-section').hide();
...
function enviarMensaje() {
    ...
    if (t) {
        var r = globalChatRoom || ((currentRoom && currentRoom !== "") ? currentRoom : 'general');
...
function handleBotClick(s) {
    ...
    movimientoComun(s, () => {
        if (!game.game_over()) botTimeout = setTimeout(() => {
            var m = game.moves();
